...
(function ($) {
    ...
    $.DiySelecter = function (t, opt) {
        ...
        var init = function () {
            ...
            // fill up fake options by ajax or from origin select
            if (opt.ajaxUrl) {
                $.ajax({
                    type: 'GET',
                    url: opt.ajaxUrl,
            ...
            // TODO: trigger it's hasLayout property in IE6.
            if (isIE6) { $li.css("zoom", "1"); }
        };
        ...
        var displayUL = function () {
            ...
            $fakeOption.css({
                // in some IE versions, it appears to get incorrect value with $.offset, use $.position instead
                left: $fakeSelect.position().left,
                top: (dt > db && opt.autoPosition) ?
    ...
    $.DiySelecter.defaults = {
        ...
        checkText: '',                  // default text to display
        width: 0,                       // custom width of fake select, 0 means not custom
        autoWidth: true,
        circleSelect: true,
        autoPosition: true,
        ajaxUrl: null,
        ...
        parseData: function (data) {},  // when using ajax to fill up options, ur own method to parse data
